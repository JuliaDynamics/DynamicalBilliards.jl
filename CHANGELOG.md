...
* Many low-level functions are not exported any more, for safety and because it didn't make much sense: `propagate!`, `relocate!`, `resolvecollision!`, `periodicity!`, `specular!` `realangle`. For the low level interface only `bounce!`, `collision` and `next_collision` are exposed. The low level interface is _still_ documented though.
